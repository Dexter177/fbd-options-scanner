...
def analyse_ticker(
    ...
    try:
        ...
        if anchor and anchor > 0:
            ...
            if current < anch:
                out["fbd_status"] = f"⚠️ Still below level (${anch:.2f}) — not yet recovered"
            elif lo_5d > anch:
                out["fbd_status"] = f"⚠️ 5-day low (${lo_5d:.2f}) never touched level — not a flush"
            else:
                out["fbd_status"] = (
